...
// Stand: 2025-07-23_16:30:00, Codezeilen: ~70 (Hinzugefuegt: updateKategorieFlags)
...
interface KategorieDao {
    ...
    /**
     * NEU: Synchrone Methode zum Abrufen einer Kategorie nach ID (fuer interne Repository-Logik).
     * @param kategorieId Die ID der abzurufenden Kategorie.
     * @return Die Kategorie-Entitaet oder null, falls nicht gefunden.
     */
    @Query("SELECT * FROM kategorie WHERE kategorieId = :kategorieId")
    suspend fun getKategorieByIdSynchronous(kategorieId: String): KategorieEntitaet?
     ...
     */
    @Query("SELECT * FROM kategorie WHERE istLoeschungVorgemerkt = 0 ORDER BY reihenfolge ASC")
    fun getAllKategorien(): Flow<List<KategorieEntitaet>>

    /**
     * Ruft alle Kategorien ab, einschliesslich derer, die zur Loeschung vorgemerkt sind.
     * @return Eine Liste von Kategorie-Entitaeten.
     */
    ...
    /**
     * NEU: Aktualisiert die isLokalGeaendert und isLoeschungVorgemerkt Flags direkt in der Datenbank.
     * Dies ist nuetzlich, nachdem ein Sync mit Firestore erfolgreich war und die Flags zurueckgesetzt werden muessen.
     */
    @Query("UPDATE kategorie SET istLokalGeaendert = :istLokalGeaendert, istLoeschungVorgemerkt = :istLoeschungVorgemerkt WHERE kategorieId = :kategorieId")
    suspend fun updateKategorieFlags(kategorieId: String, istLokalGeaendert: Boolean, istLoeschungVorgemerkt: Boolean)
}
//// app/src/main/java/com/MaFiSoft/BuyPal/data/KategorieDao.kt
