...
// Stand: 2025-07-27_21:30:00, Codezeilen: ~680 (Migrationslogik in migriereAnonymeProduktGeschaeftVerbindungen korrigiert)
...
class ProduktGeschaeftVerbindungRepositoryImpl @Inject constructor(
    ...
    override suspend fun verbindungSpeichern(verbindung: ProduktGeschaeftVerbindungEntitaet) {
        ...
        val verbindungToSave = verbindung.copy(
            erstellungszeitpunkt = existingVerbindung?.erstellungszeitpunkt ?: verbindung.erstellungszeitpunkt, // Behalte bestehenden Zeitstempel bei, falls vorhanden
        ...
        try {
            if (existingVerbindung != null) {
                produktGeschaeftVerbindungDao.verbindungAktualisieren(verbindungToSave)
                Timber.d("$TAG: Verbindung ProduktID='${verbindungToSave.produktId}', GeschaeftID='${verbindungToSave.geschaeftId}' lokal AKTUALISIERT. istLokalGeaendert: ${verbindungToSave.istLokalGeaendert}")
            } else {
                produktGeschaeftVerbindungDao.verbindungEinfuegen(verbindungToSave)
                Timber.d("$TAG: Verbindung ProduktID='${verbindungToSave.produktId}', GeschaeftID='${verbindungToSave.geschaeftId}' lokal EINGEFUEGT. istLokalGeaendert: ${verbindungToSave.istLokalGeaendert}")
            }
            ...
            Timber.e(e, "$TAG: FEHLER (verbindungSpeichern): Ausnahme beim lokalen Speichern/Aktualisieren der Verbindung: ${e.localizedMessage ?: e.message}")
    ...
    override suspend fun markVerbindungForDeletion(produktId: String, geschaeftId: String) {
        ...
        if (existingVerbindung != null) {
            ...
            try {
                ...
                Timber.e(e, "$TAG: FEHLER (markVerbindungForDeletion): Ausnahme beim lokalen Vormerken der Verbindung zur Loeschung: ${e.localizedMessage ?: e.message}")
                throw e
            }
        } else {
    ...
    override suspend fun markiereAlleVerbindungenFuerProduktZurLoeschung(produktId: String) {
        ...
        // getVerbindungenByProduktId gibt einen Flow zurück, wir brauchen den aktuellen Wert
    ...
    override suspend fun migriereAnonymeProduktGeschaeftVerbindungen(neuerBenutzerId: String) {
        ...
        try {
            ...
            anonymeVerbindungen.forEach { verbindung ->
                ...
                // WICHTIG: Verwende verbindungAktualisieren, da es sich um eine bestehende Verbindung handelt
                produktGeschaeftVerbindungDao.verbindungAktualisieren(aktualisierteVerbindung)
    ...
    private suspend fun performPullSync() {
        ...
        try {
            ...
            val relevantArtikelIds = mutableSetOf<String>() // Nicht direkt für PGV-Pull benötigt, aber für Produkt-Relevanz
